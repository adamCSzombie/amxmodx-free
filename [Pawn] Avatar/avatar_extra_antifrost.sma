...
new g_antifrost[ 33 ], g_cooldown[ 33 ], i_cooldown_time[ 33 ], g_itemid, g_iHudSync, maxplayers;
new Float:anti_frost = 30.0;

public plugin_natives( )
{
	register_native( "is_user_antifrost","native_is_user_antifrost",1 )
}
...
public client_putinserver( id ) g_antifrost[ id ] = false;

public client_disconnect( id ) g_antifrost[ id ] = false;

public zp_extra_item_selected( player, itemid )
{
	if ( itemid == g_itemid )
	{
		if ( g_antifrost[ player ] )
		{
			client_cmd( player, "spk valve/sound/buttons/button11" );
			ChatColor( player, "%L", LANG_PLAYER, "ANTI_FROST_HAVE" );
			return ZP_PLUGIN_HANDLED;		
		}
		
		g_antifrost[ player ] = true;
		i_cooldown_time[ player ] = floatround( anti_frost );		
		set_task( 1.0, "ShowHUD", player, _, _, "a",i_cooldown_time[ player ] );
		g_cooldown[ player ] = 1;
		ChatColor( player, "%L", LANG_PLAYER, "ANTI_FROST" );
	}
...
{	
	if( g_antifrost[ victim ] )
	{
		g_antifrost[ victim ] = false;
		remove_task( victim );
	}
}
...
public ShowHUD( id )
{
	...
	if( g_antifrost[ id ] )
	{
		...
		{
			g_antifrost[ id ] = false;
			client_print( id, print_center, "%L", LANG_PLAYER, "ANTI_FROST_END" )
		}
		...
		if( i_cooldown_time[ id ] >= 0 )
		{
			...
			ShowSyncHudMsg( id, g_iHudSync, "Anti-Frost: %d sec.",i_cooldown_time[ id ] );
		}
		else
		{
			client_print( id, print_center, "%L", LANG_PLAYER, "ANTI_FROST_END" )
