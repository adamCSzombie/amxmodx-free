...
public plugin_init() 
{
	...
	g_iItemId = zp_register_extra_item("Plasma Zbran \w[\rExtraVIP\w]", g_Cost, ZP_TEAM_HUMAN)
...
public zp_extra_item_selected(id, itemid)
{
	...
	{	
		if( !is_user_hannibal( id ) ) {
			...
			zp_set_user_ammo_packs(id, zp_get_user_ammo_packs(id) + g_Cost)
			client_print( id, print_center, "Ked si hannibal nemozes kupit Plasmu!" );
			return PLUGIN_CONTINUE
		}
	}
	return PLUGIN_CONTINUE
...
public primary_attack(id)
{
	...
	{	
		...
		if(zp_get_user_zombie(iTarget) || zp_get_user_nemesis(iTarget))
		{
			...
			if(iBody == HIT_HEAD)
			{
				set_hudmessage( 255, 42, 42, -1.0, 0.55, 2, 0.1, 0.4, 0.02, 0.02, -1)
				show_hudmessage( id, "-240 HP" );
				iDamage = DAMAGE
				iBloodScale = 10
			}
			else
