...
{
	"models/bluezone/zombie/v_flamethrower.mdl",
	"models/bluezone/zombie/p_flamethrower.mdl",
	"models/bluezone/zombie/w_flamethrower.mdl"
}
...
new g_smokepuff_id, g_weapon_event, g_register,cvar_cost
...
public plugin_init() 
{
	...
	g_salamander = zp_register_extra_item( "Plamenomet \w[\rExtraVIP\w]", 200, ZP_TEAM_HUMAN );
...
public zp_extra_item_selected(id, itemid)
{
	if(itemid == g_salamander) 
	{
		...
		{
			client_print( id, print_center,"Ked si Hannibal nemozes si kupit Plamenomet!" )
			zp_set_user_ammo_packs( id, zp_get_user_ammo_packs( id ) + 200 )
			return;
		}
		if ( have_user_minigun( id ) )
		{
			client_print( id, print_center,"Nemozes kupit plamenomet ked mas MiniGun!" )
			zp_set_user_ammo_packs( id, zp_get_user_ammo_packs( id ) + 200 )
			return;
		}
		get_salamander(id)
...
{
	register_native("have_user_salamander","native_is_user_hannibal",1)
}
