...
#define PLUGIN 		"[Avatar] Bazooka s Nabijanim"
#define VERSION 	"0.3"
#define AUTHOR 		"adamCSzombie"
#define EXTRA_ITEM	"[\yVIP\w] Bazooka"
...
static const mrocket[ ] 	= "models/gamesites/bazooka/zm_bazooka_missile.mdl";
static const mrpg_w[ ] 	= "models/gamesites/bazooka/w_zm_bazooka.mdl";
static const mrpg_v[ ] 	= "models/gamesites/bazooka/v_zm_bazooka.mdl";
static const mrpg_p[ ] 	= "models/gamesites/bazooka/p_zm_bazooka.mdl";
#else
static const mrocket[ ] 	= "models/gamesites/bazooka/zm_bazooka_missile.mdl";
static const mrpg_w[ ] 	= "models/gamesites/bazooka/w_zm_bazooka.mdl";
static const mrpg_v[ ] 	= "models/gamesites/bazooka/v_zm_bazooka.mdl";
...
public plugin_init( )
{
	...
	pcvar_maxdmg 		= register_cvar( "av_bazooka_damage_n", BAZOOKA_DAMAGE );
	...
	if ( itemid == g_itemid )
	{
		if( get_user_flags( player ) & ADMIN_LEVEL_H )
		{
			if( g_bazooka_kolo[ player ] )
			{
				...
				ChatColor( player, "!g[Avatar]!y Muzes koupit pouze jednu Bazooku za kolo!" );
				zp_set_user_ammo_packs(player, zp_get_user_ammo_packs(player) + 160 )
			...
			{
				client_cmd( player, "spk valve/sound/buttons/button11" );
				ChatColor( player, "!g[Avatar]!y Uz mas zakoupenou Bazooku!" );
				zp_set_user_ammo_packs(player, zp_get_user_ammo_packs(player) + 160 )
				return ZP_PLUGIN_HANDLED;		
			}
			else if( is_user_granatomet( player ) )
			{
				client_cmd( player, "spk valve/sound/buttons/button11" );
				client_print(player, print_center,"Pokud vlastnis Bazooku, nemuzes mit Granatomet!" );
				zp_set_user_ammo_packs(player, zp_get_user_ammo_packs(player) + 160 )
			...
			{
				client_cmd( player, "spk valve/sound/buttons/button11" );
				ChatColor( player, "!g[Avatar]!y Muzes vlastnit pouze jednu Bazooku!" );
				return ZP_PLUGIN_HANDLED;
			}
			else 
			{
				...
				g_cooldown[ player ] = 0;
				ChatColor( player, "!g[Avatar]!y Mas nabitou Bazooku!" );
		...
		{
			client_cmd( player, "spk valve/sound/buttons/button11" );
			client_print( player, print_chat, "Tento item je pouze pro VIP hrace!", player );
			zp_set_user_ammo_packs( player, zp_get_user_ammo_packs( player ) + 160 );
		}
	}
	return PLUGIN_CONTINUE;
...
baz_damage( id, attacker, damage, weaponDescription[ ] )
{
	...
	set_task( 0.3, "give_body", attacker );
}
		...
		if ( is_user_alive( id ) )
		{
			switch( mode[ id ] ) 
			{
				...
				case 2:
				{
					mode[ id ] = 3
					client_cmd( id, "spk valve/sound/fvox/beep" );
					client_print( id, print_center, "[ Rizena strela ]" );
				}
				case 3:
				{
					mode[id] = 1
					client_cmd( id, "spk valve/sound/fvox/beep" );
					client_print( id, print_center, "[ Normalni strela ]" );
				}		
			}	
...
public ShowHUD( id )
{
	...
	if(i_cooldown_time[ id ] >= 0)
	{
		...
		ShowSyncHudMsg( id, g_iHudSync, "Bazooka se nabiji: %d sec.",i_cooldown_time[ id ] );
	}
	else
	{
		CanShoot[ id ] = true;
