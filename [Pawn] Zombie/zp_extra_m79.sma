...
#define RELOAD_TIME		4.3 // Don't set this lower that 3.0
#define REFIRE_RATE		4.2 // Refire rate
...
public plugin_precache ( )
{
	...
	m_iExplo = precache_model ( "sprites/m79_explode.spr" )
}
...
{
	// New plugin
	register_plugin ( "[ZP] Extra Item:M79", VERSION, "adamCSzombie" )
	
	// Game-Monitor support
	register_cvar ( "zp_m79_version", VERSION, FCVAR_FLAGS )
	
	// New extra items
	g_m79 = zp_register_extra_item ( "M79 Grenader [\rExtraVIP\w]", 250, ZP_TEAM_HUMAN )
	...
	// CVARs
	cvar_maxdmg = register_cvar ( "zp_m79_maxdmg_n", "900" )
	cvar_radius = register_cvar ( "zp_m79_radiusd_n", "600" )
...
public zp_extra_item_selected ( Player, Item )
{
	...
	if ( Item == g_m79 )
	{
		if ( is_user_hannibal( Player ) ) {
			ChatColor( Player, "!g[ZP]!y Ked si !gHannibal!y nemozes mit !gGranatomet!" );
