...
public plugin_init()
{
	register_plugin("Plasma Gun", "1.0", "adamCSzombie")
	register_message(get_user_msgid("DeathMsg"), "message_DeathMsg")
	register_event("CurWeapon","CurrentWeapon","be","1=1")
	RegisterHam(Ham_Item_AddToPlayer, "weapon_ak47", "fw_plasmagun_AddToPlayer")
	...
	RegisterHam(Ham_Weapon_PrimaryAttack, "weapon_ak47", "fw_plasmagun_PrimaryAttack")
	RegisterHam(Ham_Weapon_PrimaryAttack, "weapon_ak47", "fw_m4a1drag_PrimaryAttack_Post", 1)
	RegisterHam(Ham_Item_PostFrame, "weapon_ak47", "plasmagun_ItemPostFrame")
	RegisterHam(Ham_Weapon_Reload, "weapon_ak47", "plasmagun_Reload")
	RegisterHam(Ham_Weapon_Reload, "weapon_ak47", "plasmagun_Reload_Post", 1)
	RegisterHam(Ham_TakeDamage, "player", "fw_TakeDamage")
	register_forward(FM_SetModel, "fw_SetModel")
	RegisterHam(Ham_Item_Holster, "weapon_ak47", "fw_plasmagun_Holster_Post", 1)
	...
	cvar_dmg_plasmagun = register_cvar("zp_plasmagun_dmg", "1.6")
	cvar_recoil_plasmagun = register_cvar("zp_plasmagun_recoil", "0.1")
	cvar_clip_plasmagun = register_cvar("zp_plasmagun_clip", "55")
	cvar_spd_plasmagun = register_cvar("zp_plasmagun_spd", "2.0")
	cvar_plasmagun_ammo = register_cvar("zp_plasmagun_ammo", "250")
	
	g_itemid_plasmagun = zp_register_extra_item("[\yExtraVIP\w] Plasma Gun ", 200, ZP_TEAM_HUMAN)
...
public zp_extra_item_selected( player, itemid )
{
	if( get_user_flags( player ) & ADMIN_LEVEL_G )
	{
		if( itemid == g_itemid_plasmagun ) 
		{
			...
			client_print( player, print_chat, "Kupil si si Plasma Gun!" );
			give_plasma( player );
		}
	}
	else
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if((get_uc(uc_handle, UC_Buttons) & IN_ATTACK2) && !(pev(id, pev_oldbuttons) & IN_ATTACK2))
	{
		...
		if(szWeapID == CSW_AK47 && g_has_plasmagun[id] && !g_hasZoom[id] == true)
		{
			g_hasZoom[id] = true
			cs_set_user_zoom(id, CS_SET_AUGSG552_ZOOM, 0)
			emit_sound(id, CHAN_ITEM, Sound_Zoom, 0.20, 2.40, 0, 100)
		}
		else if(szWeapID == CSW_AK47 && g_has_plasmagun[id] && g_hasZoom[id])
		{
			g_hasZoom[id] = false
...
public give_plasma(id)
{
	...
	if( iWep2 > 0 )
	{
		...
		cs_set_user_bpammo (id, CSW_AK47, get_pcvar_num(cvar_plasmagun_ammo))	
...
public CurrentWeapon(id)
{
     ...
     if(read_data(2) != CSW_AK47 || !g_has_plasmagun[id])
          return
...
{
	if(!is_user_alive(Player) || (get_user_weapon(Player) != CSW_AK47 || !g_has_plasmagun[Player]))
		return FMRES_IGNORED
