...
#define CVAR_JUMPS	"av_supervojak_skokydgagag"
#define MAX_JUMPS	"3"
...
{
	register_native( "is_user_supervojak","native_is_user_supervojak",1 )
}
...
public plugin_init( )
{
	...
	formatex( text, charsmax( text ), "%L", LANG_PLAYER, "SUPER_VOJAK" );
...
public native_is_user_supervojak( id )
{
	if ( !is_user_valid( id ) )
	{
	...
	if( itemid == g_uqz )
	{
		if( limit_term != 50 )
		{
			...
			{	
				...
				if( is_user_supermarinak( player ) )
				{
					client_cmd( player, "spk valve/sound/buttons/button11" );
					client_print( player, print_chat, "%L", LANG_PLAYER, "SUPER_HAVE" );
					zp_set_user_ammo_packs( player, zp_get_user_ammo_packs( player ) + ITEM_COST );
					return;
				}
				...
				if( spustac[ player ] == 1 )
				{
					...
					{	
						...
						ChatColor( player, "%L", LANG_PLAYER, "SUPER_BUY" );
						set_user_health( player, get_user_health( player ) + 350 ); 
					...
					{
						client_cmd( player, "spk valve/sound/buttons/button11" );
						client_print( player, print_chat, "%L", LANG_PLAYER, "SUPER_LIMIT" ); 
						zp_set_user_ammo_packs( player, zp_get_user_ammo_packs( player ) + ITEM_COST );
						return;
					}
				}
				...
				set_user_rendering( player, kRenderFxGlowShell, 54, 102, 246, kRenderNormal, 10 );		
				cmd_luk( player );			
				...
				ShowSyncHudMsg( 0, g_msgSync,"%L", LANG_PLAYER, "SUPER_IS", name );
			}
			else
			{
				client_cmd( player, "spk valve/sound/buttons/button11" );
...
public event_round_start( )
{
	for( new i = 1; i <= g_iMaxPlayers; i++ )
	{
		...
		if( spustac[ i ] )
		{
			...
			client_print( i, print_center, "%L", LANG_PLAYER, "SUPER_END" );
			spustac[ i ] = 0;
			g_norecoil[ i ] = false;
		}
...
public cmd_luk( id )
{
	...
	{
		new text[ 555 char ];
		formatex( text, charsmax( text ), "%L", LANG_PLAYER, "SUPER_WEAPON" );
