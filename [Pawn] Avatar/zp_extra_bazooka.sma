...
new g_cooldown[ 33 ], i_cooldown_time[ 33 ];

new Float:nabijanie = 45.0; // Nabijanie Bazooky

#define BAZOOKA_DELAY		"45"
#define BAZOOKA_DAMAGE		"2650"
...
public plugin_init( )
{
	...
	pcvar_maxdmg 		= register_cvar( "av_bazooka_damage", BAZOOKA_DAMAGE );
...
public zp_extra_item_selected( player, itemid )
{
	if ( itemid == g_itemid )
	{
		...
		{
			ChatColor( player, "!g[Avatar]!y Uz mas zakupenu Bazooku!" );
			return ZP_PLUGIN_HANDLED;		
		}
		else if ( baz_count( ) > get_pcvar_num( pcvar_count ) )
		{
			ChatColor( player, "!g[Avatar]!y Nemozes mat naraz tolko Bazooky!" );
			return ZP_PLUGIN_HANDLED;
		}
		else 
		{
			...
			g_cooldown[ player ] = 0;
			ChatColor( player, "!g[Avatar]!y Mas pripravenu Bazooku!" );
...
public give_bazooka( id, level, cid )
{
	...
	{
		console_print( id,"Nemas na toto pravo" );
		return;
	}
	if ( read_argc() > 2 ) 
	{
